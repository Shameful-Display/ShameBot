...
test('memeRepliesModule koolaidReply sends image and sets presence', () => {
  ...
  const p = path.join('modules', 'memeImages', 'koolaid.jpg');
  assert.ok(sent[0].attachment.path.includes(p)
    || sent[0].attachment.path.includes('modules/memeImages/koolaid.jpg'));
});